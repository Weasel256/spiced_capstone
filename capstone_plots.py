...
def barplot(y_true, y_pred, file_name=None, classify_case=False):
    ...
    if classify_case:
        pred_label = 'correctly classified'
        seizure_label = 'seizure'
        non_seizure_label = 'no seizure'
    else:
    ...
    # df['amount'] = df['amount'] / len(y_true) * 100 # make it relative
    df.head()
    ...
    if classify_case:
        plt.ylabel('Number of EEG segments', fontsize=18, fontweight='bold', color=text_color)
    else:
        plt.ylabel('Number of EEG sequences', fontsize=18, fontweight='bold', color=text_color)
...
def plot_roc(y_true, y_pred, file_name=None):
    ...
    plt.xlabel('False positive rate', fontsize=16, color=text_color)
    plt.ylabel('True positive rate', fontsize=16, color=text_color)
